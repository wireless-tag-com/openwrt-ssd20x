...
static MS_BOOL _Hal_GetMsTime(unsigned long tPreTime, MS_U32 u32Fac)
{
    if (time_after_eq(jiffies, (tPreTime + u32Fac)))
        return TRUE;
    return FALSE;
}
...
static MS_BOOL _HAL_SERFLASH_WaitWriteCmdRdy(void)
{
    ...
    } while (!SER_FLASH_EXPIRE(ts, MAX_READY_WAIT_JIFFIES));
...
MS_BOOL HAL_SERFLASH_DMA(MS_U32 u32FlashStart, MS_U32 u32DRASstart, MS_U32 u32Size)
{
    ...
    } while (!SER_FLASH_EXPIRE(ts, msecs_to_jiffies(u32Timeout));
