...
#define RX_DELAY_INT            1
    ...
    #define TX_QUEUE_CNT_SCHE   (0)     // 0 : SW patch with EMAC1_0x24
					// 1 : EMAC0_0x6C
...
#define JULIAN_104_VAL                      (0x30010000UL)
#else
...
typedef struct
{
    int         (*phy_write)(void*, u8, u8, u32);
    int         (*phy_read)(void*, u8, u8, u32*);
...
static int _MHal_EMAC_albany_write(void* hal, u8 phy_addr, u8 addr, u32 val);
static int _MHal_EMAC_albany_read(void* hal, u8 phy_addr, u8 addr, u32* val);
...
static int _MHal_EMAC_ext_write(void* hal, u8 phy_addr, u8 addr, u32 val);
static int _MHal_EMAC_ext_read(void* hal, u8 phy_addr, u8 addr, u32* val);
...
//-------------------------------------------------------------------------------------------------
int MHal_EMAC_write_phy(void* hal, unsigned char phy_addr, unsigned char address, u32 value )
{
...
// Note: MDI interface is assumed to already have been enabled.
//-------------------------------------------------------------------------------------------------
int MHal_EMAC_read_phy(void* hal, unsigned char phy_addr, unsigned char address, u32* value )
{
...
int MHal_EMAC_QueueFree_4(void* hal)
{
    u32 tsrval = 0;
    u8  avlfifo[8] = {0};
    u8  avlfifoidx;
...
u8 MHal_EMAC_QueueUsed_New(void* hal)
{
...
///////////////// TXQ
static int _MHal_EMAC_TXQ_Size(void* hal)
{
    hal = hal;
    return TX_QUEUE_SIZE;
}

static int _MHal_EMAC_TXQ_Free(void* hal)
{
    int ret;
#if (4 == TX_QUEUE_SIZE)
...
static int _MHal_EMAC_TXQ_Used(void* hal)
{
    int ret;
...
static int _MHal_EMAC_TXQ_Empty(void* hal)
{
    return (0 == _MHal_EMAC_TXQ_Used(hal)) ? 1 : 0;
}

static int _MHal_EMAC_TXQ_Full(void* hal)
{
    return (0 == _MHal_EMAC_TXQ_Free(hal)) ? 1 : 0;
}

static int _MHal_EMAC_TXQ_Insert(void* hal, u32 bus, u32 len)
{
...
int _MHal_EMAC_TXQ_Mode(void* hal)
{
    hal = hal;
    return 0;
}
