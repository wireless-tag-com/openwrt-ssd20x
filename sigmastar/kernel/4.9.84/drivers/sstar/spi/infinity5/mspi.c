...
//  RegbaseAddr  Disc
//-------------------------------------------------------------------------------------------------
//#define WAIT_DONE_BY_ISR
#define WAIT_DONE_BY_BUSY_POLL
...
{
#ifdef WAIT_DONE_BY_BUSY_POLL
	unsigned long start;
#endif
	unsigned long timeout;

    // trigger operation
#ifdef WAIT_DONE_BY_ISR
    reinit_completion(&bs->done);
#endif
    MSPI_WRITE(MSPI_TRIGGER_OFFSET,MSPI_TRIGGER);

#ifdef WAIT_DONE_BY_ISR
    timeout = wait_for_completion_timeout(&bs->done, msecs_to_jiffies(MSTAR_SPI_TIMEOUT_MS));
#endif

#ifdef WAIT_DONE_BY_BUSY_POLL
    start = jiffies;
    timeout = start + msecs_to_jiffies(MSTAR_SPI_TIMEOUT_MS);
    while(!(_HAL_MSPI_CheckDone(bs) & 1)) {
        //usleep_range(5, 10);
